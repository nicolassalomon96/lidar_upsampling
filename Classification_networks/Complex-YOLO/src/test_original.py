...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for batch_idx, (img_paths, imgs_bev) in enumerate(test_dataloader):
            ...
            #################################### CREATE LABEL FOLDER + FILES ############################################
            label_path = os.path.join(configs.working_dir, 'dataset', 'kitti', 'testing', 'labels_gen')
            label_name = img_paths[0].split(sep='\\')[-1].split(sep='.')[0] + '.txt'
            
            if not os.path.exists(label_path):
                os.mkdir(label_path)

            with open(os.path.join(label_path, label_name), 'w') as f: 
                for objs in objects_pred:
                    f.write(objs.to_kitti_format())               
