...
from data_gen_kitti_carla import *
...
writer_path = r'.\models_result\model_cnn1d_min_82-12_Chamfer_sinnorm_no_ego_filtered_2048_kitti_CARLA.txt'
...
#train_dataset = IterDataset(train_distance_data_generator)
#valid_dataset = IterDataset(valid_distance_data_generator)
...
'''
class Approx_net(nn.Module):    
    #def __init__(self, input_channels, input_features, conv_filters_1, conv_filters_2, conv_filters_3, linear_features):
    def __init__(self, input_channels, input_features, conv_filters_1, conv_filters_2, linear_features):
        ...
        #self.conv3 = nn.Conv1d(in_channels=conv_filters_2, out_channels=conv_filters_3, kernel_size=1)
        # hidden layer
...
'''
class Approx_net(nn.Module):    
    def __init__(self, input_channels, input_features, conv_filters_1, conv_filters_2, conv_filters_3):
        super(Approx_net, self).__init__()
        #cnn layer
        self.conv1 = nn.Conv1d(in_channels=input_channels, out_channels=conv_filters_1, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=conv_filters_1, out_channels=conv_filters_2, kernel_size=1)
        self.conv3 = nn.Conv1d(in_channels=conv_filters_2, out_channels=2, kernel_size=1)
        # hidden layer
        #self.linear_1 = nn.Linear(in_features=conv_filters_2*input_features+1, out_features=linear_features) #Para considerar el mínimo, se suma 1 a la entrada
        #self.linear_1 = nn.Linear(in_features=conv_filters_2*input_features, out_features=linear_features)
        self.linear_2 = nn.Linear(in_features=input_features*conv_filters_3, out_features=1)
    ...
    def forward(self, x):
        ...
        x_ = self.conv2(x_)
        x_ = self.conv3(x_)
        #print(f'Salida con1d-3: {x_.shape}')
        x_ = self.flatten(x_)
        #x_ = x_.view(x.size(0), -1)
        #x_ = torch.hstack((x_, min_value.unsqueeze(1)))
        #print(f'Entrada MLP: {x_.shape}')
        ...
        return y
'''
#mlp_net = Approx_net(input_channels=1, input_features=6, conv_filters_1=8, conv_filters_2=4, conv_filters_3=2)
#mlp_net = Approx_net(input_channels=1, input_features=6, conv_filters_1=10, conv_filters_2=8, conv_filters_3=6, linear_features=12)
...
######################################### LOSS ######################################
# Definir la pérdida SSIM
#class SSIMLoss(piqa.SSIM):
#    def forward(self, x, y):
#        return 1. - super().forward(x.repeat(1, 3, 1, 1).clamp(0.0,1.0), y.repeat(1, 3, 1, 1))
...
#loss_fn = SSIMLoss(window_size=11).cuda()

############################## OPTIMIZER - LR SCHEDULER ###############################

#optimizer = torch.optim.RMSprop(mlp_net.parameters(), lr=lr, weight_decay=0)
optimizer = torch.optim.Adam(mlp_net.parameters(), lr=lr, weight_decay=1e-5)
#optimizer = torch.optim.Adam(list(mlp_net.parameters()) + list(loss_fn.parameters()), lr=lr, weight_decay=lr/100)
...
def train_one_epoch(epoch_index):
    ...
    for j, data in enumerate(train_dataloader):
        #inicio = time.time()
        lrimg_distance, hrimg_distance  = data
        ...
        windows = get_windows(lrimg_distance)
        ...
        pixels = pixels.view(lrimg_distance.shape[0], lrimg_distance.shape[1], -1, lrimg_distance.shape[-1])

        real_pixels = hrimg_distance[:,:,1:hrimg_distance.shape[2]-1:2, :hrimg_distance.shape[3]]
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for k, vdata in enumerate(valid_dataloader):
            vlrimg_distance, vhrimg_distance  = vdata
            vlrimg_distance = vlrimg_distance.to(device)
            vhrimg_distance = vhrimg_distance.to(device)

            vwindows = get_windows(vlrimg_distance)                  
            ...
            vpixels = vpixels.view(vlrimg_distance.shape[0], vlrimg_distance.shape[1], -1, vlrimg_distance.shape[-1])

            real_vpixels = vhrimg_distance[:,:,1:vhrimg_distance.shape[2]-1:2, :vhrimg_distance.shape[3]]
    ...
    fin = time.time()
    print(f'Epoch {epoch_number + 1} - Train_loss: {avg_loss} - Valid_loss: {avg_vloss} - lr: {optimizer.param_groups[0]["lr"]} - Tiempo: {np.round((fin-inicio)/60.0,3)} minutos - Estimado: {np.round(((EPOCHS-epoch_number-1)*(fin-inicio))/60.0,3)} minutos')
    #print(f'Epoch {epoch_number + 1} - Train_loss: {avg_loss} - Valid_loss: {avg_vloss} - gamma: {list(loss_fn.parameters())[0].item()} - Tiempo: {np.round((fin-inicio)/60.0,3)} minutos - Estimado: {np.round(((EPOCHS-epoch_number-1)*(fin-inicio))/60.0,3)} minutos')
    # Track best performance, and save the model's state
    if avg_vloss < best_vloss:
        best_vloss = avg_vloss
        model_path = rf'.\models_result\model_cnn1d_min_82-12_Chamfer_sinnorm_no_ego_filtered_2048_kitti_CARLA.pth'
