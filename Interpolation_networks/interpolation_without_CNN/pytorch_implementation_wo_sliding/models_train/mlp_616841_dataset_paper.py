#MODELO ENTRENADO CON DATOS OBTENIDOS DEL DATASET DEL PAPER SIN REDES NEURONALES.
#PARA ACELERAR EL ENTRENAMIENTO, YA QUE LO QUE SE BUSCA ES APROXIMAR LA FUNCIÓN DE INTERPOLACIÓN EMPLEADA EN EL PAPER
#SE GENERARON LAS VENTANAS OFFLINE Y SERIALIZARON LOS DATOS PARA INGRESAR A LA RED SOLAMENTE CON LOS PIXELES DESEADOS,
#SIN TENER QUE REALIZAR EL ALGORITMO DE LA VENTANA DESLIZANTE.
...
writer_path = r'..\models_result\model_816841_dataset_paper_logger.txt'
...
mlp_net = Approx_net(input_size=6, hidden_neurons_1=16, hidden_neurons_2=8, hidden_neurons_3=4, output_size=1)
...
def train_one_epoch(epoch_index):
    ...
    for j, data in enumerate(train_dataloader):
        ...
        windows = get_windows(lrimgs)
        windows_stack = windows.reshape(windows.shape[0] * windows.shape[1], windows.shape[2])
        outputs = mlp_net(windows_stack).squeeze()
        #outputs_squeeze = outputs.squeeze()
         
        real_pixels = hrimgs[:,:,1:hrimgs.shape[2]-1:2, :hrimgs.shape[3]].flatten()
        #print(real_pixels.shape)

        # Compute the loss and its gradients
        loss = loss_fn(outputs, real_pixels)
        #print(loss)
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for k, vdata in enumerate(valid_dataloader):
            ...
            windows = get_windows(vlrimgs)
            windows_stack = windows.reshape(windows.shape[0] * windows.shape[1], windows.shape[2])
            voutputs = mlp_net(windows_stack)
            voutputs = voutputs.squeeze()
         
            real_vpixels = vhrimgs[:,:,1:vhrimgs.shape[2]-1:2, :vhrimgs.shape[3]].flatten()
            
            vloss = loss_fn(voutputs, real_vpixels)
    ...
    if avg_vloss < best_vloss:
        ...
        model_path = rf'..\models_result\model_816841_dataset_paper_sin_CNN_ep{epoch_number+1}.pth'
...
model_path = rf'..\models_result\model_816841_dataset_paper_sin_CNN_ep{epoch_number}.pth'
torch.save(mlp_net.state_dict(), model_path)