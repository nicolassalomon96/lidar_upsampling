...
class ChamferLoss(nn.Module):
    def __init__(self, device, normalized_out=True, consider_out_image_points=True, separate_pointcloud=True):
        super(ChamferLoss, self).__init__()
        self.device = device
        self.normalized_out = normalized_out #Considerar las nubes de puntos normalizadas en [-1,1] para el cálculo del error
        self.consider_out_image_points = consider_out_image_points #Considerar todos los puntos de la nube (incluso los que no estan en la vista de la cámara RGB y etiquetados)
        self.separate_pointcloud = separate_pointcloud #Considerar el error separando la nube entre nube con objetos y sin objetos
        self.gamma = 2.0 #Constante para ponderar una nube de puntos sobre otra al calcular el error
        self.squared_chamfer = False
    ...
    def forward(self, image_pred, image_gt, label_path):
        ...
        if self.separate_pointcloud:
            pointcloud_gt_non_filtered, pointcloud_gt_filtered = pointcloud_filter(pointcloud_gt, label_path, normalized_output=self.normalized_out, consider_out_image_points=self.consider_out_image_points) #Listas de tamaño [batch_size] con las nubes de puntos
            pointcloud_pred_non_filtered, pointcloud_pred_filtered = pointcloud_filter(pointcloud_pred, label_path, normalized_output=self.normalized_out, consider_out_image_points=self.consider_out_image_points)
            #print(len(pointcloud_pred_filtered), pointcloud_pred_filtered[0].shape, len(pointcloud_gt_filtered), pointcloud_gt_filtered[0].shape)
            ...
            for i in range(len(label_path)):
                ...
                #if chamfer_distance_filtered.isnan():
                    #chamfer_distance_filtered = torch.tensor([100.0], device=self.device, requires_grad=True)
                #if chamfer_distance_non_filtered.isnan():
                #    chamfer_distance_non_filtered = torch.tensor([100.0], device=self.device, requires_grad=True)
                #chamfer_loss_filtered.append(chamfer_distance_filtered)
                #chamfer_loss_non_filtered.append(chamfer_distance_non_filtered)
                
                #if len(pointcloud_pred_filtered[i]) > 0:
                    # Pointclouds must have dimensions [batch, num_points, 3]
                    #print(pointcloud_pred_filtered[i][:,:3].max(), pointcloud_pred_filtered[i][:,:3].min())
                #    chamfer_distance_filtered = kaolin.metrics.pointcloud.chamfer_distance(pointcloud_pred_filtered[i][:,:3].unsqueeze(0).to(self.device), pointcloud_gt_filtered[i][:,:3].unsqueeze(0).to(self.device), self.squared_chamfer)
                #    chamfer_distance_non_filtered = kaolin.metrics.pointcloud.chamfer_distance(pointcloud_pred_non_filtered[i][:,:3].unsqueeze(0).to(self.device), pointcloud_gt_non_filtered[i][:,:3].unsqueeze(0).to(self.device), self.squared_chamfer)
                ...
                #else:
                #    chamfer_distance_filtered = torch.tensor([100.0], device=self.device, requires_grad=True)
                    #Si no hay puntos dentro del bbox, es porque todos los puntos estan por fuera, por lo tanto se puede calcular una distancia chamfer que no dé valores elevados
                #    chamfer_distance_non_filtered = kaolin.metrics.pointcloud.chamfer_distance(pointcloud_pred_non_filtered[i][:,:3].unsqueeze(0).to(self.device), pointcloud_gt_non_filtered[i][:,:3].unsqueeze(0).to(self.device), self.squared_chamfer)

                chamfer_loss_filtered.append(chamfer_distance_filtered)
                chamfer_loss_non_filtered.append(chamfer_distance_non_filtered)
            
            #cantidad_de_nan = sum(tensor.isnan() for tensor in chamfer_loss_filtered)
            #print("Cantidad de NaN en la lista:", cantidad_de_nan)
            chamfer_loss_filtered = [torch.tensor([100.0], device=self.device, requires_grad=True) if x.isnan() else x for x in chamfer_loss_filtered]
            ...
            #print(chamfer_loss_filtered_tensor.requires_grad)
            #print(chamfer_loss_non_filtered_tensor.requires_grad)

            #chamfer_loss = torch.mean(chamfer_loss_filtered_tensor) * self.gamma + torch.mean(chamfer_loss_non_filtered_tensor)
            chamfer_loss = chamfer_loss_filtered_tensor * self.gamma + chamfer_loss_non_filtered_tensor
            #chamfer_loss_clamp = torch.clamp(chamfer_loss, max=200.0)
            ...
            chamfer_loss = kaolin.metrics.pointcloud.chamfer_distance(pointcloud_pred[:,:,:3].to(self.device), pointcloud_gt[:,:,:3].to(self.device), self.squared_chamfer)
...
new_pixel_coords = torch.Tensor(list(zip(odd_row_pos, column_pos))[:-1024]) #[:-1024] #Posiciones (x,y) de cada nuevo pixel a interpolar
...
def train_one_epoch(epoch_index, new_pixel_coords):
    ...
    for j, data in enumerate(train_dataloader):
        ...
        #print(f'Los pesos son iguales: {torch.equal(before.data, after.data)}') #Para comprobar si los pesos se actualizan
