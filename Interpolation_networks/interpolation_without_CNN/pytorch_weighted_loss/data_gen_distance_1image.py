...
velodyne_name = os.listdir(velodyne_folder_1image) #lr images and hr images have the same name
labels_name = os.listdir(labels_folder) #folder with all labels


#Train/Valid
train_urls = velodyne_name
valid_urls = velodyne_name
...
def train_data_generator():
    ...
    for _, url in enumerate(train_urls):
        ...
        #Get random images
        train_path = os.path.join(velodyne_folder_1image, url) #hr_folder + '\\' + url
...
def valid_data_generator():
    ...
    for _, url in enumerate(valid_urls):
        ...
        #Get random images
        valid_path = os.path.join(velodyne_folder_1image, url) #hr_folder + '\\' + url
        if valid_path[-3:] == 'npy':
