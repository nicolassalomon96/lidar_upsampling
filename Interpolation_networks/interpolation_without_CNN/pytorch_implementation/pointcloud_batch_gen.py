...
pointcloud_saved_path = r'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\Complex-YOLO\dataset\kitti\testing\velodyne_64_mired_dataset_paper'
...
model = Approx_net(input_size=8, hidden_neurons_1=16, hidden_neurons_2=8, hidden_neurons_3=4, output_size=1)
...
for i, data in enumerate(tqdm(dataloader)):
    ...
    #pointcloud_distance = range_image_to_pointcloud_pytorch(lrimg_distance, device)
    
    #for j, pointcloud in enumerate(pointcloud_distance):
    #    pointcloud_join = torch.hstack([pointcloud[:,:3], lrimg_intensity[j,0,:,:].reshape(-1,1)])
    #    save_path = os.path.join(pointcloud_saved_path, f'{str(pointcloud_number).zfill(6)}.bin')
    #    save_bin(pointcloud_join.cpu().detach().numpy(), save_path)
    #    pointcloud_number += 1
