...
import time
import kaolin
from data_gen_distance_intensity_v2 import *
...
class Approx_net(nn.Module):    
    def __init__(self, input_size, hidden_neurons_1, hidden_neurons_2, hidden_neurons_3, hidden_neurons_4, output_size):
...
mlp_net = Approx_net(input_size=14, hidden_neurons_1=20, hidden_neurons_2=10, hidden_neurons_3=6, hidden_neurons_4=8, output_size=1)
...
#optimizer = torch.optim.Adam(mlp_net.parameters(), lr=lr, weight_decay=0.001)
...
def get_windows(x, new_pixel_coords, upscaling_factor=2):
    ...
    if upscaling_factor == 2:
        ...
        #windows_intensity_mean = torch.mean(windows_intensity, 2, True)
        windows = torch.dstack((windows_distance, windows_intensity, new_pixel_coords_batch))     
    else:
        print("ERROR: Wrong Upsampling factor")
    return windows
...
def train_one_epoch(epoch_index, new_pixel_coords):
    ...
    for j, data in enumerate(train_dataloader):
        ...
        pixels = pixels.view(lrimgs.shape[0], 1, -1, lrimgs.shape[-1])  
        
        real_pixels = hrimgs[:,:1,1:hrimgs.shape[2]-1:2, :hrimgs.shape[3]]
...
for epoch in range(EPOCHS):
    ...
    net_parameters.append(torch.mean(mlp_net.linear_three.bias).item())
    ...
    with torch.no_grad():
        for k, vdata in enumerate(valid_dataloader):
            ...
            vpixels = vpixels.view(vlrimgs.shape[0], 1, -1, vlrimgs.shape[-1])
        
            real_vpixels = vhrimgs[:,:1,1:vhrimgs.shape[2]-1:2, :vhrimgs.shape[3]]
    ...
    if avg_vloss < best_vloss:
        ...
        model_path = rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\Scripts\otras_arquitecturas\3_pytorch_interpolation\results\mlp_8201061\model_Leaky_ReLU_8201061_30ep_fulldataset_Chamfer\model_Leaky_ReLU_86432161_reduced_Chamfer_Rprop_Cosine_ep{epoch_number+1}.pth'
        torch.save(mlp_net.state_dict(), model_path)

    train_parameters.append(f'Epoch {epoch_number + 1} - Train_loss: {avg_loss} - Valid_loss: {avg_vloss}')
    parameters_writer(train_parameters, writer_path)

    epoch_number += 1

model_path = rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\Scripts\otras_arquitecturas\3_pytorch_interpolation\results\mlp_8201061\model_Leaky_ReLU_8201061_30ep_fulldataset_Chamfer\model_Leaky_ReLU_86432161_reduced_Chamfer_Rprop_Cosine_ep{epoch_number}.pth'
torch.save(mlp_net.state_dict(), model_path)