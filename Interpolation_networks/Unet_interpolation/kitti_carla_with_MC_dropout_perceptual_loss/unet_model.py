...
def create_vgg(n_layers):
### Create Loss Model (VGG16) ###
    selectedLayers = range(n_layers + 1) #Primeras 8 capas hasta ReLu3_3
    vgg_model = tf.keras.applications.vgg16.VGG16(include_top=False, weights='imagenet', input_shape=(high_res, image_columns, 3))
...
# Custom VGG16 loss function
def perceptual_loss(y_true, y_pred):

    y_true = keras.layers.Concatenate(axis=-1)([y_true, y_true, y_true]) #Gray to RGB --> Para usar con VGG16
    y_pred = keras.layers.Concatenate(axis=-1)([y_pred, y_pred, y_pred])
    
    y_true_vgg = vgg(y_true)
    y_pred_vgg = vgg(y_pred)
    #loss = perceptual_loss_lambda * tf.reduce_mean((y_pred_vgg[n_layers] - y_true_vgg[n_layers])**2)
    loss = perceptual_loss_lambda * tf.reduce_mean(tf.math.abs(y_true_vgg[n_layers] - y_pred_vgg[n_layers]))
    return loss
...
def Unet():
    ...
    #model.compile(optimizer=keras.optimizers.Adam(learning_rate=adam_lr), loss='mae')
    #model.compile(optimizer=keras.optimizers.Adam(learning_rate=adam_lr), loss=ssim_loss)
    #model.compile(optimizer=keras.optimizers.Adam(learning_rate=adam_lr), loss=cosine_sim_loss)
    #model.compile(optimizer=keras.optimizers.Adam(learning_rate=adam_lr), loss=huber_loss)  
    model.compile(optimizer=keras.optimizers.Adam(learning_rate=adam_lr), loss=perceptual_loss, metrics=[ssim_metric])
