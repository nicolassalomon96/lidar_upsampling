...
def train_data_generator():
    ...
    while True:
        for _, url in enumerate(train_urls):
            ...
            lrimg = io.imread(lr_folder + '\\' + train_urls[idx])
            hrimg = io.imread(hr_folder + '\\' + train_urls[idx])
            
            lrimg, hrimg = data_augmentation(lrimg, hrimg, dataset, mode)
            #hrimg = np.concatenate((hrimg,hrimg,hrimg),axis=-1)
            yield (lrimg, hrimg)
...
def valid_data_generator():
    ...
    while True:
        for _,url in enumerate(valid_urls):
            ...
            lrimg = io.imread(lr_folder + '\\' + valid_urls[idx])
            hrimg = io.imread(hr_folder + '\\' + valid_urls[idx])
