...
if __name__ == '__main__':
  ...
  # load point clouds
  #scan_dir = rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\Datasets LIDAR\kitti\kitti_3d_object\training\velodyne\{file_id}.bin'
...
#
  ...
  # load point clouds
  scan_dir = rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\PointPillars\dataset\velodyne_64_cnn1d_min_82-12_Chamfer_MAE_no_ego_filtered\training\velodyne\{file_id}.bin'
  scan = np.fromfile(scan_dir, dtype=np.float32).reshape(-1, 4)

  # load image
  img = np.array(io.imread(rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\PointPillars\dataset\velodyne_64_cnn1d_min_82-12_Chamfer_MAE_no_ego_filtered\training\image_2\{file_id}.png'), dtype=np.int32)

  # load labels
  with open(rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\PointPillars\eval_results\results_midataset_cnn1d_Chamfer_MAE_no_ego_filtered\submit\{file_id}.txt', 'r') as f:
    labels = f.readlines()

  # load calibration file
  with open(rf'D:\Nicolas\Posgrado\Trabajos y Tesis\LIDAR\LIDAR_super_resolution\PointPillars\dataset\velodyne_64_cnn1d_min_82-12_Chamfer_MAE_no_ego_filtered\training\calib\{file_id}.txt', 'r') as f:
    lines = f.readlines()
    P2 = np.array(lines[2].strip().split(' ')[1:], dtype=np.float32).reshape(3, 4)
    R0 = np.array(lines[4].strip().split(' ')[1:], dtype=np.float32).reshape(3, 3)
    V2C = np.array(lines[5].strip().split(' ')[1:], dtype=np.float32).reshape(3, 4)
